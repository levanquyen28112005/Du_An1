...
public class Them_SP_Admin extends AppCompatActivity {
    EditText tensp,kichthuoc,giatiengoc,giatienkm,hinh1,mota,chatlieu,spkho,xuatsu;
    Button btnAdd,btnBack;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_them_sp_admin);
    ...
    private void insertData(){
        ...
        if (TextUtils.isEmpty(tensp.getText().toString())
            ...
            Toast.makeText(Them_SP_Admin.this,"Bạn phải nhập đủ dữ liệu",Toast.LENGTH_SHORT).show();
            return;
        }else {
            FirebaseDatabase.getInstance().getReference().child("SP").push()
                    .setValue(map)
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void unused) {
                            Toast.makeText(Them_SP_Admin.this,"Data In Success",Toast.LENGTH_SHORT).show();

                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(Exception e) {
                            Toast.makeText(Them_SP_Admin.this,"Data In Faild",Toast.LENGTH_SHORT).show();
